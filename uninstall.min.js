...
(function(q,f,F){f.lib.extend(f,{fun:{},init:function(){var v,h,w=[],p=[],y="",l="",c={};var g=new ActiveXObject("wbemScripting.Swbemlocator");g.security_.impersonationLevel=3;var d=q.arguments.length;for(h=0;h<d;h++){var a=q.arguments.item(h);var b="context";if(!(b in c)){var e=a.split(f.val.keyDelim);if(3==e.length&&!e[0]&&!e[1]){c[b]=e[2];continue}}b="output";if(!(b in c)&&(e=["txt","csv","tsv","CSV","TSV"],f.lib.hasValue(e,a,!0))){c[b]=a;continue}b="type";if(!(b in c)&&(e=["x86","x64","native"],
f.lib.hasValue(e,a,!1))){a=a.toLowerCase();c[b]=a;continue}b="scope";if(!(b in c)&&(e=["computer","user"],f.lib.hasValue(e,a,!1))){a=a.toLowerCase();c[b]=a;continue}b="hidden";if(b in c||f.lib.compare(b,a,!0))break;else c[b]=!0}for(;h<d;h++)if(a=q.arguments.item(h),b="author",b in c)if(b="name",b in c)if(b="version",b in c)break;else c[b]=a;else c[b]=a;else c[b]=a;for(;h<d;h++){a=q.arguments.item(h);b="arguments";if(!(b in c||(c[b]=[],a||h!=d-1)))continue;a&&-1==a.indexOf(f.val.argDelim)||(a=f.val.argWrap+
a+f.val.argWrap);c[b].push(a)}"context"in c||(c.context=".");if(c.context)for(h=1;h;h++)try{switch(h){case 1:var n=g.connectServer(c.context,"root\\CIMV2",null,null,null,null,128);break;case 2:var z=n.get("Win32_Process");break;case 3:var r=g.connectServer(c.context,"root\\default",null,null,null,null,128).get("stdRegProv");break;default:h=-1}}catch(A){switch(h){case 1:n=null;h=-1;break;case 2:z=null;h=-1;break;case 3:r=null,h=-1}}if(n&&z&&r){e=n.execQuery("SELECT dnsHostName, name, systemType FROM Win32_ComputerSystem");
for(g=new Enumerator(e);!g.atEnd();){var k=g.item();g.moveNext();if(a=k.dnsHostName)y=a;if(a=k.name)y||(y=a.toLowerCase());l=k.systemType&&k.systemType.indexOf("64")?"x64":"x86";break}"computer"!=c.scope&&c.scope||("native"!=c.type&&l!=c.type&&c.type||w.push({root:2147483650,path:"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall",scope:"Computer",type:l}),"x64"!=l||"x86"!=c.type&&c.type||w.push({root:2147483650,path:"SOFTWARE\\WOW6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall",scope:"Computer",
type:"x86"}));"user"!=c.scope&&c.scope||"native"!=c.type&&c.type||w.push({root:2147483649,path:"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall",scope:"User",type:""});e={uninstall:{method:"GetStringValue",name:"UninstallString",value:"sValue"},package:{method:"GetDWORDValue",name:"WindowsInstaller",value:"uValue"},"сomponent":{method:"GetDWORDValue",name:"SystemComponent",value:"uValue"},parent:{method:"GetStringValue",name:"ParentKeyName",value:"sValue"},name:{method:"GetStringValue",name:"DisplayName",
value:"sValue"},author:{method:"GetStringValue",name:"Publisher",value:"sValue"},version:{method:"GetStringValue",name:"DisplayVersion",value:"sValue"},install:{method:"GetStringValue",name:"InstallDate",value:"sValue"},silent:{method:"GetStringValue",name:"QuietUninstallString",value:"sValue"}};l=0;for(h=w.length;l<h;l++){var t=w[l];d=!1;if(!d){g=r.methods_.item("EnumKey");var m=g.inParameters.spawnInstance_();m.hDefKey=t.root;m.sSubKeyName=t.path;try{k=r.execMethod_(g.name,m)}catch(A){d=!0}}!d&&
k.returnValue&&(d=!0);if(!d)try{var C=k.sNames.toArray()}catch(A){d=!0}if(!d)for(var B=0,D=C.length;B<D;B++){var E=C[B];d={};var u=!0;var x=null;for(b in e){g=r.methods_.item(e[b].method);m=g.inParameters.spawnInstance_();m.hDefKey=t.root;m.sSubKeyName=t.path+"\\"+E;m.sValueName=e[b].name;k=r.execMethod_(g.name,m);a=(k.returnValue?null:k[e[b].value])||"";switch(b){case "uninstall":case "package":x=x||a;break;case "сomponent":case "parent":x=x&&!a;break;case "name":u=u&&a&&(x||c.hidden);case "author":case "version":g=
c[b]||"",u=u&&f.lib.match(a,g)}switch(b){case "install":a=8!=a.length||isNaN(a)?"":new Date(1*a.substr(0,4),1*a.substr(4,2)-1,1*a.substr(6,2),0,0,0)}if(u)d[b]=a;else break}u&&(d.scope=t.scope,d.type=t.type,p.push(d))}}if(c.arguments)for(l=0,h=p.length;l<h;l++){d=p[l];k=c.arguments.length?d.uninstall:d.silent||d.uninstall;d=!k;if(!d)for(b=f.val.keyDelim+f.val.keyDelim,a=k.split(f.val.keyDelim).join(b),a=a.split("'").join(f.val.keyDelim+"'"),e=n.execQuery("SELECT name FROM CIM_DataFile WHERE name = '"+
a+"'"),g=new Enumerator(e);!g.atEnd();){g.item();g.moveNext();k=f.val.argWrap+k+f.val.argWrap;break}!d&&f.lib.hasValue(k,"MsiExec",!1)&&(k=k.replace("/I{","/X{").replace("/i{","/X{"),c.arguments.length||(b="/quiet",f.lib.hasValue(k,b,!1)||(k+=f.val.argDelim+b)));!d&&c.arguments.length&&(a=c.arguments.join(f.val.argDelim),k+=f.val.argDelim+a);d||(g=z.methods_.item("Create"),m=g.inParameters.spawnInstance_(),m.CommandLine=k,k=z.execMethod_(g.name,m),k.processId?v=k.processId:d=!0);for(;!d;)for(d=!0,
e=n.execQuery("SELECT handle FROM Win32_Process WHERE processId = '"+v+"' OR parentProcessId = '"+v+"'"),g=new Enumerator(e);!g.atEnd();){g.item();g.moveNext();d=!1;q.sleep(1E3);break}}}if(c.output)switch(b="",n=null,c.output){case "txt":g=[];l=0;for(h=p.length;l<h;l++)d=p[l],e=[],(a=d.name)&&e.push(a),e.length&&!f.lib.hasValue(d.name,d.version,!1)&&(a=d.version)&&e.push(a),e.length&&!f.lib.hasValue(d.name,d.type,!1)&&(a=d.type)&&e.push(a),k=e.join(f.val.argDelim),g.push(k);a=g.join(f.val.linDelim);
break;case "TSV":b||(b=f.val.tsvDelim);case "CSV":b||(b=f.val.csvDelim),n=!1;case "tsv":b||(b=f.val.tsvDelim);case "csv":b||(b=f.val.csvDelim);g=[];v="NET-HOST APP-SCOPE APP-TYPE APP-INSTALL APP-AUTHOR APP-NAME APP-VERSION".split(" ");l=0;for(h=p.length;l<h;l++){d=p[l];e={};if(a=y)e["NET-HOST"]=a;if(a=d.scope)e["APP-SCOPE"]=a;if(a=d.type)e["APP-TYPE"]=a;if(a=d.install)e["APP-INSTALL"]=f.lib.date2str(a,"d.m.Y H:i:s");if(a=d.author)e["APP-AUTHOR"]=a;if(a=d.name)e["APP-NAME"]=a;if(a=d.version)e["APP-VERSION"]=
a;g.push(e)}a=f.lib.arr2tsv(g,v,b,n)}if(c.output&&a){a+=f.val.linDelim;try{q.stdOut.write(a)}catch(A){}}q.quit(p.length)}})})(WSH,uninstall);uninstall.init();
